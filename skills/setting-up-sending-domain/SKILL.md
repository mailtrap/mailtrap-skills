...
**Related skills:** `sending-emails` (after domain is ready).
...
1. **DNS records and status via API** — Use the Sending Domains API:
  - `GET https://mailtrap.io/api/accounts/{account_id}/sending_domains` — lists domains
  - `GET https://mailtrap.io/api/accounts/{account_id}/sending_domains/{sending_domain_id}` — returns `dns_records` (each with `type`, `name`, `value`, and verification `status`) and `dns_verified`. Poll after you publish DNS.
2. **Create domain via API** —
  - `POST https://mailtrap.io/api/accounts/{account_id}/sending_domains` with `domain_name` when your flow provisions domains programmatically.
